...
public class RegisterController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(email == null || email.isEmpty() || password == null || password.isEmpty()) {
            request.setAttribute("error_empty", "Vui lòng điền đầy đủ thông tin.");
            request.getRequestDispatcher("/WEB-INF/views/userpages/register.jsp").forward(request, response);

            return;
        }
        if(!email.matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
            request.setAttribute("error_email_format", "Định dạng email không hợp lệ.");
            request.getRequestDispatcher("/WEB-INF/views/userpages/register.jsp").forward(request, response);
            return;
        }
        if(!password.matches(
            ...
            request.setAttribute("error_password_format",
                    "Mật khẩu phải ≥ 8 ký tự, có chữ hoa, chữ thường và số");
            request.getRequestDispatcher("/WEB-INF/views/userpages/register.jsp").forward(request, response);
        }

        if(!password.equals(passwordConfirm)) {
            request.setAttribute("error_password", "Mật khẩu xác nhận không khớp.");
            request.getRequestDispatcher("/WEB-INF/views/userpages/register.jsp").forward(request, response);
